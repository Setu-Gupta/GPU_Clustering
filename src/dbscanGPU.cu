...
__device__
void print_bool_arr(unsigned char *a, int n)
{
	for(int i = 0; i < n; i++)
		if(a[i] == 1)
			printf("%d ", i);
...
__global__
void print_graph(int *V, int *edges, int *indices, int n)
{
	for(int i = 0; i < n; i++)
	{
		for(int j = 0; j < V[i]; j++)
...
__global__
void kernel_init_uint(unsigned int* a,int size)
{
	int tx = threadIdx.x + (blockDim.x * blockIdx.x);
	if(tx < size)
		a[tx] = 0;
						...
						unsigned int* map, unsigned char* visited, int* workToDo)
{
	...
	kernel_init_uint<<<n_grid_dim, THREADS_PER_BLOCK>>>(map, n);
